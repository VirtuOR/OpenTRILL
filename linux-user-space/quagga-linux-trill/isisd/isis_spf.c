...
trill_complete_spf(struct isis_area *area)
{
  ...
  retval = isis_run_spf (area, TRILL_ISIS_LEVEL, AF_PACKET, isis->sysid, NULL);
  if (retval != ISIS_OK)
  ...
  for (ALL_DICT_NODES_RO(area->trill->nickdb, dnode, tnode))
    {
      retval = isis_run_spf (area, TRILL_ISIS_LEVEL, AF_PACKET,
